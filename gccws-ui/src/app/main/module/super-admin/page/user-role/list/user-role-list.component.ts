...
import {UserRoleAddDialogComponent} from "../dialog/user-role-add-dialog/user-role-add-dialog.component";
...
export class UserRoleListComponent implements OnInit{
  ...
  total: number = 0;
  searchValue: string = '';
  dataSource = new MatTableDataSource(new Array<UserRoleModel>());
  displayedColumns: string[] = ['expanded','slNo', 'name', 'banglaName', 'action'];

  /* Expandable table element*/
  expandedElement?: UserRoleModel | null;
  ...
  openDeleteDialog(row: UserRoleModel): void{
  ...
  openAddDialog(isEdit: boolean, model?: UserRoleModel, ): void{
    model = model ? model : new UserRoleModel();
    const dialogConfig = this.appUtils.getDialogConfig();
    dialogConfig.data = {
      model: model,
      isEdit: isEdit,
    };
    const dialogRef = this.matDialog.open(UserRoleAddDialogComponent, dialogConfig);
